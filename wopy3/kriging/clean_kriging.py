...
def stationary_likelihood_func(pd,Z,pars,covar_func=C_sph_nugget,covariates=None,
    ...
    mu = Z.mean()
    if use_scipy:
        mu = np.ones(len(Z)) * Z.mean()
        dist_obj = stats.multivariate_normal(mu,sigma,allow_singular=True)
        return dist_obj.logpdf(Z)
    else:
        try:
...
def MLE_radius_bayes(x,y,vals,x0,hyperpars,radius=10.0,minSill=0.0,
                    minNugget=0.0,maxRange=180.0,prior="inv_gamma",
                    covariates=None,use_scipy=False):
    ...
    if not isinstance(prior,str):
        ...
        prior_func = lambda theta,hyperpars: log_inv_gamma(theta,hyperpars)
    elif prior =="gamma":
        prior_func = lambda theta,hyperpars:log_gamma(theta,hyperpars)
    elif prior =="normal":
        prior_func = lambda theta,hyperpars:log_gauss(theta,hyperpars)
    
    func = lambda theta:-stationary_likelihood_func(pd,vals[in_circle],theta,covariates=covariates,use_scipy=use_scipy)-prior_func(theta,hyperpars)
...
class MLEKrigor:
    ...
    def __init__(self,x,y,vals,cat=None,covariates=None,use_scipy=False):
        """
        Parameters
        ----------
        x,y,vals : np.array

...
# -*- coding: utf-8 -*-
"""
Modifed from on 20/09/2024 for AIDMAP
...
def igrf_cal(magdata):
    IGRF_FILE = r'./IGRF14.shc'
    igrf = load_shcfile(IGRF_FILE, None)
    alt, colat = gg_to_geo(magdata.altitude/1000, 90-magdata.lat)
    
    f = interpolate.interp1d(igrf.time, igrf.coeffs, fill_value='extrapolate')
    coeffs = f(magdata.time)   

    Br, Bt, Bp = synth_values(coeffs.T, alt, colat, magdata.lon, igrf.parameters['nmax'])
    dec, hoz, inc, eff = xyz2dhif(-Bt,Bp,-Br)
    magdata_igrf=magdata.assign(dec=dec, inc=inc, total_field=eff)
